...
lambda_hc_ss = 12.879268210397512;
...
phi_l2 = 1.609735517273332e+02;
pi_ss = 1;
PP_ss = 1;
psilSS = 6.943698158831134;
Pstar_ss = 1;
QhF_ss = 2.981163775734776;
Qhss = 1;
QK_ss = 1;
R_ss = 0.010101010101010;
re_ss = 0.010101010101010;
resi_derr_ss = 1.981163775734776;
...
// suppose default probaility is fixed  time invariant, and we drop shocks e4 and e5
//sigmaomegae_v = e4 + rhosigmaomegae_ss*sigmaomegae_v(-1);
//sigmaomegah_v = e5 + rhosigmaomegah_ss*sigmaomegah_v(-1);
// determinstic 
sigmaomegae_v = sigmaomegae_ss ; 
sigmaomegah_v = sigmaomegah_ss ; 
...
lambda_hc_v  -  zetc_v  +  PP_v  +  cc_v  -  bb2 *cc_v(-1) + bb2*bbeta*zetc_v(+1) -  PP_v   ;
...
//
v_v  = -  omegae_v(+1)  -  kappae_v(+1)   +  omegae_v(+1)  +   RK_v(+1)   +   QK_v  +  K_v -  Nw_v  +  Nw_v +  Nw_v +  R_v(+1) -  R_v(+1)  + QK_v  +   K_v ;
...
//QhF_v = - chih*hh_v - chih*lambda_hc_v +  kappah_v  +  Qh_v  +  lambda_hc_v  -  re_v -  lambda_hc_v + bbeta*lambda_hc_v(+1)   - bbeta*lambda_hc_v  - bbeta*Qh_v(+1)  ;
//QhF_v = - chih*hh_v  - (chih+ bbeta)*lambda_hc_v  +  kappah_v  +  Qh_v   -  re_v + bbeta*lambda_hc_v(+1)    - bbeta*Qh_v(+1)  ;
...
save irf_plot_with_policy001 c_v_e7 c_v_e6 Y_v_e6 Y_v_e7 L_v_e6 L_v_e7 m_v_e6 m_v_e7 d_h_v_e6 d_h_v_e7 Y_v_e19 L_v_e19 c_v_e19 d_h_v_e19 m_v_e19 omegae_v_e19 omegae_v_e6 omegah_v_e19 omegah_v_e7;
//c_v_e5 
//c_v_e4...
//Y_v_e4 Y_v_e5 
//L_v_e4 L_v_e5 
...
y_IRF_both_nofric001 = y2(:,M_.maximum_lag+1:end)-repmat(oo_.dr.ys,1,n_points);


save y_IRF_both_nofric001 ;
...
y2 = simult_(M_,options_,initial_condition_states,oo_.dr,shock_matrix,1);
y_IRF_both_noprudential001 = y2(:,M_.maximum_lag+1:end)-repmat(oo_.dr.ys,1,n_points);


save y_IRF_both_noprudential001;
...
stoch_simul(order=1, irf = 40, ar=0, periods=0, nograph, noprint, irf_shocks=(e1,e2,e3,e6,e7,e8)); % e5,e4
...
// ----
rhosigmaomegae_ss_vec =  normrnd(0.271451374116692,7.50701190099790e-09,1,H);
rhosigmaomegah_ss_vec =  normrnd(0.410825051226036,5.82787862413170e-08,1,H);
rhosigmakappae_ss_vec =  normrnd(0.733024196303945,1.67190705724352e-11,1,H);
rhosigmakappah_ss_vec =  normrnd(0.146761017576996,2.46984654950211e-11,1,H);
rhoggg_vec  =  normrnd(0.753579386242248,5.00222085975111e-12,1,H);
rhozz_vec =  normrnd(0.295894075162782,1.00257580015750e-11,1,H);
rhozetc_vec =  normrnd(0.977466580382832,3.74456021745573e-11,1,H);
zetap_vec = normrnd(0.691531948474610,2.55795384873636e-13,1,H);
zetal_vec = normrnd(0.868133039809254,1.08403241938504e-08,1,H);
bb_vec = normrnd(0.6459,1.67092650826817e-09,1,H);
bb2_vec = normrnd(0.507554383018338,1.88009764201524e-07,1,H);
ssii_vec  =  normrnd(5.76927289354519,4.46043202373403e-12,1,H);
alphpi_vec =  normrnd(1.40212845677026,5.12930913743048e-07,1,H);
rhop_vec =  normrnd(0.870309696134287,4.18695549342374e-08,1,H);
alphdey_vec =  normrnd(0.120172413758037,1.10074580561559e-07,1,H);
alphakomegae_vec  = normrnd(1.78377144322198,8.07405342584389e-09,1,H);
alphakomegah_vec   = normrnd(2.32216366979369, 7.89768250797351e-12,1,H);
...
stoch_simul(order=1)Y_v,m_v,d_h_v; % Store output, inflation and government variable responses
...
res_kappae_v(:,h)= omegae_v_e6*100;
